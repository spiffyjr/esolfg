...
if $yaml_values == undef {
  $yaml_values = loadyaml('/vagrant/.puppet/config.yaml')
...
if hash_key_equals($nodejs_values, 'install', 1) {
  apt::key { 'B9316A7BC7917B12': }
  apt::ppa { 'ppa:chris-lea/node.js':
    require => Apt::Key['B9316A7BC7917B12']
  }

  class { 'nodejs':
    manage_repo => $nodejs_values['manage_repo'],
    version     => $nodejs_values['version'],
  }

  each($nodejs_values['packages']) |$package_name| {
